...
const EventCard = ({ event, topPos, height, leftPos, width, onEdit, onDelete, onOpen, isGuest, isFocused, anyFocused }) => {
      ...
      <div className="calendar-container">
        ...
        <div className={`calendar-summary-panel ${!summarySidebarOpen ? 'collapsed' : ''}`}>
          ...
          <div style={{ marginBottom: '12px', paddingBottom: '12px', borderBottom: '1px solid var(--border-light)' }}>
            ...
            <p style={{ fontSize: '11px', fontWeight: '700', color: 'var(--text-secondary)', margin: '4px 0 0 0' }}>{format(weekStart, 'dd MMM', { locale: enUS })} - {format(addDays(weekStart, 6), 'dd MMM', { locale: enUS })}</p>
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {(() => {
              let globalIdx = 0;
              return CATEGORY_LIST.map(dept => {
